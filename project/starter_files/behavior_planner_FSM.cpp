...
double BehaviorPlannerFSM::get_look_ahead_distance(const State& ego_state) {
  ...
  // TODOX-Lookahead: One way to find a reasonable lookahead distance is to find
  // the distance you will need to come to a stop while traveling at speed V and
  // using a comfortable deceleration.
  VelocityProfileGenerator vp; // We have the calculation
  auto look_ahead_distance = vp.calc_distance(velocity_mag, 0, -_max_accel); //taken by word "comfortable deceleration" probably less then max_accel? leaving it for now
...
State BehaviorPlannerFSM::state_transition(const State& ego_state, State goal,
  ...
  if (_active_maneuver == FOLLOW_LANE) {
    ...
    if (is_goal_in_junction) {
      ...
      // TODOx-goal behind the stopping point: put the goal behind the stopping
      ...
      // TODOx-goal speed at stopping point: What should be the goal speed?? ==> 0! :)
      goal.velocity.x = goal.velocity.y = goal.velocity.z = 0.0;

    } else {
      // TODOx-goal speed in nominal state: What should be the goal speed now
    ...
    // TODOx-maintain the same goal when in DECEL_TO_STOP state: Make sure the
    ...
    // TODOx-use distance rather than speed: Use distance rather than speed...
    if (distance_to_stop_sign <= P_STOP_THRESHOLD_DISTANCE) {
      // if (distance_to_stop_sign <= P_STOP_THRESHOLD_DISTANCE) {
      // TODOx-move to STOPPED state: Now that we know we are close or at the
    ...
    // TODOx-maintain the same goal when in STOPPED state: Make sure the new goal
    // is the same as the previous goal. That way we keep/maintain the goal at
    // the stop line. goal = ...;
    goal = _goal;  // Keep previous goal. Stay where you are. // Fixed as above
    ...
    if (stopped_secs >= _req_stop_time && tl_state.compare("Red") != 0) {
      // TODOx-move to FOLLOW_LANE state: What state do we want to move to, when
